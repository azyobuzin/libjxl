...
public:
    ...
    WithEntropy<bool> inline read_with_entropy(const SymbolChanceBitType type, const int i = 0) {
        BitChance& ch = chances.realChances.bit(type, i);
        bool bit = rac.read_12bit_chance(ch.get_12bit());
        uint64_t entropy = 0;
        ch.estim(bit, entropy);
        updateChances(type, i, bit);
        return {bit, entropy};
    }
...
public:
    ...
    WithEntropy<int> read_int_with_entropy(FinalCompoundSymbolChances<BitChance, bits> &chancesIn, int min, int max) {
        FinalCompoundSymbolBitCoder<BitChance, RAC, bits> bitCoder(table, rac, chancesIn);
        return reader_with_entropy<bits>(bitCoder, min, max);
    }
    WithEntropy<int> read_int_with_entropy(FinalCompoundSymbolChances<BitChance, bits> &chancesIn, int nbits) {
        FinalCompoundSymbolBitCoder<BitChance, RAC, bits> bitCoder(table, rac, chancesIn);
...
//        range(rangeIn),
    ...
    WithEntropy<int> read_int_with_entropy(const Properties &properties, int min, int max) {
        if (min == max) { return {min, 0}; }
        assert(properties.size() == nb_properties);
        FinalCompoundSymbolChances<BitChance,bits> &chances = find_leaf(properties);
        return coder.read_int_with_entropy(chances, min, max);
    }

    WithEntropy<int> read_int_with_entropy(const Properties &properties, int nbits) {
        assert(properties.size() == nb_properties);
        FinalCompoundSymbolChances<BitChance,bits> &chances = find_leaf(properties);
        return coder.read_int_with_entropy(chances, nbits);
    }
