...
#define READ_WITH_ENTROPY(...) (ve = coder.read_with_entropy(__VA_ARGS__), entropy += ve.entropy, ve.value)
...
public:
    ...
    WithEntropy<int> read_int_with_entropy(int min, int len) {
        assert(len >= 0);
        if (len == 0) return {min, 0};
        ...
        WithEntropy<int> ve;
        if (bit) {
            ve = read_int_with_entropy(min+med+1, len-(med+1));
        } else {
            ve = read_int_with_entropy(min, med);
        }
        return {ve.value, ve.entropy + log4k.data[0x800]};
    }

    WithEntropy<int> read_int_with_entropy(int bits) {
        return read_int_with_entropy(0, (1<<bits)-1);
    }
};
...
template <typename SymbolCoder> WithEntropy<int> reader_with_entropy(SymbolCoder& coder, int bits) {
  int pos=0;
  int value=0;
  int b=1;
  uint64_t entropy=0;
  WithEntropy<bool> ve;
  while (pos++ < bits) {
    if (READ_WITH_ENTROPY(BIT_MANT, pos)) value += b;
    b *= 2;
  }
...
template <int bits, typename SymbolCoder> WithEntropy<int> reader_with_entropy(SymbolCoder& coder, int min, int max) {
    assert(min<=max);
    ...
    assert(min <= 0 && max >= 0); // should always be the case, because guess should always be in valid range
      ...
      if (READ_WITH_ENTROPY(BIT_ZERO)) return {0, entropy};
      if (min < 0) {
        if (max > 0) {
          sign = READ_WITH_ENTROPY(BIT_SIGN);
        } else {sign = false; }
      } else {sign = true; }
    ...
    for (; e < emax; e++) {
        ...
        // actually that cannot happen
        //if ((1 << (e+1)) > amax) break;
        if (READ_WITH_ENTROPY(BIT_EXP,(e<<1)+sign)) break;
    }
    ...
    for (int pos = e; pos>0;) {
        ...
        if (minabs1 > amax) { // 1-bit is impossible
            ...
            //bit = coder.read(BIT_MANT,pos);
            if (READ_WITH_ENTROPY(BIT_MANT,pos)) have = minabs1;
        } // else 0-bit is impossible, so bit stays 1
        else have = minabs1;
        //have |= (bit << pos);
    }
    return {(sign ? have : -have), entropy};
}
...
public:
    ...
    WithEntropy<bool> read_with_entropy(SymbolChanceBitType typ, int i = 0) {
        BitChance& bch = ctx.bit(typ, i);
        bool bit = rac.read_12bit_chance(bch.get_12bit());
        uint64_t entropy = 0;
        bch.estim(bit, entropy);
        bch.put(bit, table);
        return {bit, entropy};
    }
};
...
public:
    ...
    WithEntropy<int> read_int_with_entropy(int min, int max) {
        SimpleSymbolBitCoder<BitChance, RAC, bits> bitCoder(table, ctx, rac);
        return reader_with_entropy<bits, SimpleSymbolBitCoder<BitChance, RAC, bits>>(bitCoder, min, max);
    }
    WithEntropy<int> read_int2_with_entropy(int min, int max) {
        if (min > 0) return read_int_with_entropy(0,max-min)+min;
        else if (max<0) return read_int_with_entropy(min-max,0)+max;
        else return read_int_with_entropy(min,max);
    }
    WithEntropy<int> read_int_with_entropy(int nbits) {
        assert (nbits <= bits);
        SimpleSymbolBitCoder<BitChance, RAC, bits> bitCoder(table, ctx, rac);
        return reader_with_entropy(bitCoder, nbits);
    }
};

#undef READ_WITH_ENTROPY
