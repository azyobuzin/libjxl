...
void print_entropy(const int p, const int fr, const uint32_t r, const uint32_t c, const uint64_t entropy) {
  v_printf(2, "Decode Entropy: %d,%d,%u,%u,%" PRIu64 "\n", p, fr, r, c, entropy);
}

#define READ_INT_WITH_ENTROPY(...) (ve = coder.read_int_with_entropy(__VA_ARGS__), print_entropy(p, fr, r, c, ve.entropy), ve.value)
#define READ_INT_WITH_ENTROPY_ZL(...) (ve = coder.read_int_with_entropy(__VA_ARGS__), print_entropy(p, fr, r * (image.zoom_rowpixelsize(z) >> image.getscale()), c * (image.zoom_colpixelsize(z) >> image.getscale()), ve.entropy), ve.value)
...
void flif_decode_scanline_plane(plane_t &plane, Coder &coder, Images &images, const ColorRanges *ranges, alpha_t &alpha, Properties &properties, 
    ...
    if (r > 1 && !FRA && begin == 0 && end > 3) {
      ...
      for (; c < 2; c++) {
        ...
        ColorVal curr = READ_INT_WITH_ENTROPY(properties, min - guess, max - guess) + guess;
        plane.set(r,c, curr);
      }
      for (; c < end-1; c++) {
        if (alphazero && p<3 && alpha.get(r,c) == 0) {plane.set(r,c,predictScanlines_plane(plane,r,c, grey)); continue;}
...
//                else image.set(p,z,r,c,images[fr-1](p,z,r,c));
    ...
    if (r > 1 && r < image.rows(z)-1 && !FRA && begin == 0 && end > 3) {
      for (uint32_t c = begin; c < 2; c++) {
        ...
        ColorVal curr = READ_INT_WITH_ENTROPY_ZL(properties, min - guess, max - guess) + guess;
...
bool flif_decode_FLIF2_inner_horizontal(const int p, IO& io, FLIF_UNUSED(Rac &rac), std::vector<Coder> &coders, Images &images, const ranges_t *ranges,
                             ...
                             std::vector<Transform<IO>*> &transforms, const int invisible_predictor, const int additional_props, Progress& progress) {
    const int nump = images[0].numPlanes();
    const bool alphazero = images[0].alpha_zero_special;
    const bool FRA = (nump == 5);
...
//        GeneralPlane &alpha = nump > 3 ? images[0].getPlane(3) : null_alpha;
        if (z % 2 == 0) {
                if (images[0].getDepth() <= 8) { if (!flif_decode_FLIF2_inner_horizontal<IO,Rac,Coder,Plane<ColorVal_intern_8>,ranges_t>(p,io, rac, coders, images, ranges, beginZL, endZL, quality, scale, i, z, predictor, zoomlevels, transforms, options.invisible_predictor, options.additional_props, progress)) return false;}
#ifdef SUPPORT_HDR
                else if (images[0].getDepth() > 8) { if (!flif_decode_FLIF2_inner_horizontal<IO,Rac,Coder,Plane<ColorVal_intern_16u>,ranges_t>(p,io, rac, coders, images, ranges, beginZL, endZL, quality, scale, i, z, predictor, zoomlevels, transforms, options.invisible_predictor, options.additional_props, progress)) return false; }
#endif
        } else {
                if (images[0].getDepth() <= 8) { if (!flif_decode_FLIF2_inner_vertical<IO,Rac,Coder,Plane<ColorVal_intern_8>,ranges_t>(p,io, rac, coders, images, ranges, beginZL, endZL, quality, scale, i, z, predictor, zoomlevels, transforms, options.invisible_predictor, options.additional_props, progress)) return false;}
#ifdef SUPPORT_HDR
                else if (images[0].getDepth() > 8) { if (!flif_decode_FLIF2_inner_vertical<IO,Rac,Coder,Plane<ColorVal_intern_16u>,ranges_t>(p,io, rac, coders, images, ranges, beginZL, endZL, quality, scale, i, z, predictor, zoomlevels, transforms, options.invisible_predictor, options.additional_props, progress)) return false;}
#endif
      ...
      for (int p = options.skip_p0 ? 1 : 0; p < images[0].numPlanes(); p++) {
        if (ranges->min(p) < ranges->max(p)) {
          for (size_t fr = 0; fr < images.size(); fr++) {
              ...
              WithEntropy<int> ve = metaCoder.read_int_with_entropy(minR, ranges->max(p) - minR);
              print_entropy(p, fr, 0, 0, ve.entropy);
              image.set(p,0,0,0, ve.value);
          }
